...
const ChartButton = (props: any) => {
    return (
        <StyledAnimatedButton onClick={props.onClick} style={{
            padding: '5px 10px',
            minWidth: '180px',
            display: 'flex',
...
const generateInitialSeries = (creations: ICreationExtended[], colors: any[]) => {
    const series: any[] = [];
...
const SalesCharts = (props: ISalesChartsProps) => {
    ...
    const [selectedDailyIntervalLength, setSelectedDailyIntervalLength] = useState(15);

    const [selectedTab, setSelectedTab] = useState<Tabs>(Tabs.DailySales);

    const [dailyStartDate, setDailyStartDate] = useState(moment().subtract(selectedDailyIntervalLength, 'days'));
    const [dailyEndDate, setDailyEndDate] = useState(moment());
    const [averageIncomePerDayForCurrentPeriod, setAverageIncomePerDayForCurrentPeriod] = useState(0);
    ...
    let firstSale = props.sales.sort((a, b) => Number(moment(a.createdAt).format('x')) - Number(moment(b.createdAt).format('x')))[0];
    const firstYear = Number(moment(firstSale.createdAt).format('YYYY'));

    const years = new Array((Number(moment().format('YYYY')) - Number(moment(firstSale.createdAt).format('YYYY'))) + 1)
        .fill(null)
        .map((x: any, index: number) => x = firstYear + index);
    ...
    const loadDataForMonthlyChart = () => {
        const salesThisYear = props.sales ? props.sales.filter(x => moment(x.createdAt).format('YYYY') === currentYearToShow.format('YYYY')) : []

        const januarySales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'January');
        const februarySales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'February');
        const marchSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'March');
        const aprilSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'April');
        const maySales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'May');
        const juneSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'June');
        const julySales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'July');
        const augustSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'August');
        const septemberSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'September');
        const octoberSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'October');
        const novemberSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'November');
        const decemberSales = salesThisYear.filter(x => moment(x.createdAt).format('MMMM') === 'December');
    ...
    const loadDataForDailyChart = () => {
        ...
        let colors = chroma.scale('Accent').colors(150);
        shuffleArray(colors);

        let currentDate = dailyStartDate.clone();
        ...
        for (let counter = 0; counter <= selectedDailyIntervalLength; counter++) {
            const salesThisDay = props.sales ? props.sales.filter((sale: ISale) => moment(sale.createdAt).format(DEFAULT_MOMENT_FORMAT) === currentDate.format(DEFAULT_MOMENT_FORMAT)) : [];
            incomeForThisInterval.push(salesThisDay.reduce((t, c) => t += (c.income.cents / 100), 0));

            generatedSeries.forEach(serie => {
                const salesOfThisItemThisDay = salesThisDay.filter(sale => sale.creation.name === serie.label);
                if (salesOfThisItemThisDay.length === 0) {
                    serie.data.push(0);
                } else {
                    const totalProfitMadeForThisItemOnThisDay = salesOfThisItemThisDay.reduce((t, c) => {
                        t += c.income.cents;
                        return t;
                    }, 0);
            ...
            xAxisLabels.push(currentDate.format('MMMM').slice(0, 3) + ' ' + currentDate.format('D'));
        ...
        const totalForThisPeriod = incomeForThisInterval.reduce((t, c) => t += c, 0);

        setAverageIncomePerDayForCurrentPeriod(totalForThisPeriod / incomeForThisInterval.length);

        generatedSeries = generatedSeries.filter(s => !s.data.every((x: number) => x === 0));
        ...
        if (!loaded) {
            ...
            incomeOverTimeWorker.onmessage = function (e) {
                ...
                if (props.sales) {
                    ...
                    const daysPassedSinceFirstSale = Math.abs(moment().diff(startDate, 'days'));
                    ...
                    const data = filteredProfitOverTimeData.map((x: any) => x = Number(x.moneyMadeSoFar));
    
                    setProfitOverTimeLables(new Array(data.length).fill(null).map((x, i) => filteredProfitOverTimeData[i].currentDate));
    
                    setIncomeOverTimeLabels(new Array(data.length).fill(null).map((x, i) => x = i + 1));
                    setIncomeOverTimeData([
                        {
                            ...
                            curve: 'linear',
                            showMark: () => false,
                            valueFormatter: (v: any) => (v === null ? '' : `${round(v)} ${currency}`),
                        },
                    ]);
                }
            ...
            const profitPerItemData = props.creations.filter(x => x.totalSalesAmountAfterCut.cents > 0).sort((a, b) => b.totalSalesAmountAfterCut.cents - a.totalSalesAmountAfterCut.cents).slice(0, 100).map((c, index) => ({
                id: index,
                value: (c.totalSalesAmountAfterCut.cents / 100),
                label: c.name
            }));

            const totalProfit = props.sales ? props.sales.reduce((t, c) => t += (c.income.cents / 100), 0) : 0;
            setProfitPerItemSeries([{
                data: profitPerItemData,
                innerRadius: 0,
                paddingAngle: 0.1,
                cornerRadius: 10,
                valueFormatter: (v: any) => (v === null ? '' : `${round(Number(v.value))} ${currency}`),
                arcLabel: (item: any) => `${round((item.value / totalProfit) * 100)}%`,
                arcLabelMinAngle: 15,
    ...
    useEffect(() => {
        loadDataForDailyChart();
    }, [selectedDailyIntervalLength, dailyStartDate, averageIncomePerDayForCurrentPeriod]);
                ...
                <div>
                    {
                        seriesForThisDay.map((x, i) => (
                            <div key={i} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap: '8px', fontSize: '0.9em' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <div style={{ borderRadius: '50%', background: x.color, width: '10px', height: '10px' }} />
                                    <span>
                                        {x.label}
                                    </span>
                                </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap: '8px', fontSize: '0.9em', borderTop: '1px solid black', marginTop: '4px', paddingTop: '4px' }}>
                        <div style={{ fontWeight: 'bold' }}>
                            Total income this day
                        </div>
                        <div>
                ...
                selectedTab === Tabs.DailySales ? (
                    <ChartContainer>
                        <div style={{
                            ...
                            <div>
                                <strong>Average daily income since start:</strong> {avgDailyProfit}
                            </div>
                            <div>
                                <strong>Average daily income for this interval:</strong> {round(averageIncomePerDayForCurrentPeriod)} {currency}
                            </div>
                        </div>
                        <div style={{
                            display: 'flex',
                            ...
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    <Select options={POSSIBLE_INTERVAL_LENGHTS_FOR_DAILY_CHART} onChange={newValue => {
                                        setDailyStartDate(moment().subtract(Number(newValue), 'days'));
                            ...
                            <div style={{
                                ...
                                <IntervalChangeButton onClick={() => {
                                    setDailyStartDate(dailyStartDate.subtract(selectedDailyIntervalLength, 'days'));
                                    setDailyEndDate(dailyEndDate.subtract(selectedDailyIntervalLength, 'days'));
                                    loadDataForDailyChart();
                                }}>
                                    <KeyboardDoubleArrowLeftIcon /> Previous {selectedDailyIntervalLength} days
                                ...
                                <span style={{
                                    ...
                                    From {dailyStartDate.format(DEFAULT_MOMENT_FORMAT)} to {
                                        dailyEndDate.format(DEFAULT_MOMENT_FORMAT) === moment().format(DEFAULT_MOMENT_FORMAT) ? 'Today' : dailyEndDate.format(DEFAULT_MOMENT_FORMAT)
                                    }
                                </span>
                                <IntervalChangeButton onClick={() => {
                                    setDailyStartDate(dailyStartDate.add(selectedDailyIntervalLength, 'days'));
                                    setDailyEndDate(dailyEndDate.add(selectedDailyIntervalLength, 'days'));
                                    loadDataForDailyChart();
                                }} className={
                                    dailyEndDate.format(DEFAULT_MOMENT_FORMAT) === moment().format(DEFAULT_MOMENT_FORMAT)
                                        ? 'disabled'
                                        : ''
                                }>
                                    Next {selectedDailyIntervalLength} days <KeyboardDoubleArrowRightIcon />
                                </IntervalChangeButton>
                            </div>
                        </div>
                        <BarChart
                    ...
                    <ChartContainer>
                        <div>
                            <div style={{
                                ...
                                <div>
                                    <strong>Total income made for {moment(currentYearToShow).format('YYYY')}:</strong> {round(totalIncomeFromThisYear)} {currency}
                                </div>
                                <div>
                                    <strong>Averge income made per month for {moment(currentYearToShow).format('YYYY')}:</strong> {round(averagePerMonthThisYear)} {currency}
                                </div>
                            </div>
                            <div style={{
                                width: '100%',
                            ...
                            !loadingProfitOverTime && incomeOverTimeData ? (
                                <LineChart
                                    xAxis={[{
                                        ...
                                        label: 'Time',
                                        valueFormatter: (v: any) => (v === null ? '' : `${profitOverTimeLables[Number(v)]}`)
                                    }]}
            ...
            }
            {
                selectedTab === Tabs.ProfitPerItem ? (
                    <ChartContainer style={{
                        ...
                        <p>
                            <strong>Note:</strong> The pie chart below will only show your top 100 most profitable items. This limit is set in order to prevent slow site performance in case you have a huge amount of designs for sale
                        </p>
                        <PieChart
